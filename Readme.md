...
```
NAME            STATUS                   PORTS
alumnos-datos   Up X seconds (healthy)   0.0.0.0:5432->5432/tcp
```

### 3. Conectarse a la base de datos

```bash
docker exec -it alumnos-datos psql -U alumnos_user -d alumnos
```
...
```bash
docker exec -it alumnos-datos psql -U alumnos_user -d alumnos -c "SELECT * FROM alumnos;"
```

### Ver logs del contenedor

```bash
docker compose logs -f alumnos-datos
```

### Verificar healthcheck

```bash
docker inspect --format='{{.State.Health.Status}}' alumnos-datos
```

### Backup de la base de datos

```bash
docker exec alumnos-datos pg_dump -U alumnos_user alumnos > backup.sql
```

### Restaurar un backup

```bash
docker exec -i alumnos-datos psql -U alumnos_user -d alumnos < backup.sql
```
...
| Imagen ECR  | `alumnos-datos:latest` |
| Puerto      | `5432`                 |
...
```bash
docker build -t alumnos-datos:latest .
```
...
# Tag y push
docker tag alumnos-datos:latest <ECR_REGISTRY>/alumnos-datos:latest
docker push <ECR_REGISTRY>/alumnos-datos:latest
```
...
EC2-App  (alumnos-app:latest)    — Capa App    — Puerto 8080
   |
EC2-Datos (alumnos-datos:latest) — Capa Datos  — Puerto 5432  <-- este servicio
```
...
|---|---|---|
| Version semantica | `alumnos-datos:v1.4.0` | Rollback, trazabilidad |
| Latest | `alumnos-datos:latest` | Despliegue automatico |
...
- Hace pull de `alumnos-datos:latest` desde ECR via VPC Endpoint
